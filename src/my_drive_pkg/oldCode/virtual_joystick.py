...
class MainWindow(NodeMixin, QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.get_logger().info("virtual_joystick node STARTED")

        self.x_min = self.declare_parameter("x_min", -200.0).value
        self.x_max = self.declare_parameter("x_max", 200.0).value
        self.r_min = self.declare_parameter("r_min", -200.0).value
        self.r_max = self.declare_parameter("r_max", 200.0).value
    ...
    def init_ui(self):
        ...
        img_path = (
            os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe()))) + "/../images/crosshair.jpg"
        )
...
def main(args=None):
    ...
    ex.destroy_node()
    ex.get_logger().info("virtual_joystick node has shutdown")
    rclpy.shutdown()
